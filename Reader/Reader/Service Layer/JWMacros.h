...
#define JW_appDelegate ((AppDelegate *)[[UIApplication sharedApplication] delegate])

#ifdef DEBUG
#define JW_log( s, ... ) NSLog( @"<%@:%d> %@", [[NSString stringWithUTF8String:__FILE__] lastPathComponent], __LINE__,  [NSString stringWithFormat:(s), ##__VA_ARGS__] )
#else
#define JW_log( s, ... )
#endif

#define JW_logBounds(view) JW_log(@"%@ bounds: %@", view, NSStringFromRect([view bounds]))
#define JW_logFrame(view)  JW_log(@"%@ frame: %@", view, NSStringFromRect([view frame]))
